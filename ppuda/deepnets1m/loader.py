...
class MLPNets1M(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        if self.split == 'predefined':
            ...
            self.h5_data = None
            self.h5_file = os.path.join(nets_dir, 'mlpnets1m_%s.hdf5' % (split if split in ['train', 'search'] else 'eval'))
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'predefined':
            ...
            # if self.is_train:
                ...
                # num_params = args['num_params']['custom'] / 10 ** 6
            ...
            net_args = {}
            for key in ['fc_layers', 'fc_dim', 'inp_dim', 'out_dim']:
                net_args[key] = args[key]
