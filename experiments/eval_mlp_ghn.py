...
from ppuda.ghn.nn import MLP_GHN
# from ppuda.vision.loader import image_loader
from ppuda.deepnets1m.loader import MLPNets1M
from ppuda.deepnets1m.net import MlpNetwork
...
ghn = MLP_GHN.load(args.ckpt, debug_level=args.debug, device=ghn_device, verbose=True)
...
#                                  arch=args.arch)
graphs_queue = MLPNets1M.loader(1,
                                    split='train',
                                 nets_dir=args.data_dir,
                                 large_images=False,
                                 virtual_edges=virtual_edges,
...
model = MlpNetwork(**graphs[0].net_args)
# ghn(model, graphs.to_device(ghn_device))
...
for i in range(10):
    ...
    total_reward = 0
    while not done:
        obs = torch.from_numpy(obs).float().to(ghn_device)
        action = model(obs)
